...
export class GoogleOAuthService {
  ...
  private async findOrCreateUser(
    ...
    let user = await this.mongoService.authUser.findFirst({
      where: {
        provider: 'google',
    ...
    if (!user) {
      ...
      const existingUserWithEmail =
        await this.mongoService.authUser.findUnique({
      ...
      user = await this.mongoService.$transaction(async (tx) => {
        // Create auth user
  ...
  private async logLoginAttempt(data: {
    ...
    try {
      await this.mongoService.loginHistory.create({
        data: {
          authId: data.authId,
          ipAddress: data.ip,
