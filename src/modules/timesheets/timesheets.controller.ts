...
import { CreateTimesheetDto, CreateTimesheetUnlockRequestDto, ListTimesheetsQueryDto, UpdateTimesheetStatusDto, UpdateTimesheetUnlockRequestDto } from './dto/timesheet.dto';
...
export class TimesheetsController {
  ...
  @Get('my/eligible') @Roles(UserRole.User) eligibleOwn(@CurrentUser() user: AuthenticatedUser, @Query('claimDate') claimDate?: string) { return this.service.eligibleOwn(user.userId, claimDate); }
  @Post('my/unlock-requests') @Roles(UserRole.User) requestUnlock(@CurrentUser() user: AuthenticatedUser, @Body() dto: CreateTimesheetUnlockRequestDto) { return this.service.requestUnlock(user.userId, dto); }
  ...
  @Get('unlock-requests') @Roles(UserRole.Administrator, UserRole.HR) listUnlockRequests(@Query('status') status?: any) { return this.service.listUnlockRequests(status); }
  @Patch('unlock-requests/:id') @Roles(UserRole.Administrator, UserRole.HR) reviewUnlockRequest(@Param('id') id: string, @CurrentUser() user: AuthenticatedUser, @Body() dto: UpdateTimesheetUnlockRequestDto) { return this.service.reviewUnlockRequest(id, dto.status, user.userId); }
